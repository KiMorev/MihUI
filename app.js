...
function resourceMonitorNeedsConfigChanges(sources = null, services = null) {
  ...
  return Object.entries(RESOURCE_MONITOR_RULES).some(([service, rules]) => rules.some((parts) => (
    enabledKeys.has(service)
      ? !hasResourceMonitorRule(parts)
      : hasManagedResourceMonitorRule(parts, service)
  )));
}
