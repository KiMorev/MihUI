...
const els = {
  ...
  providerCreateFormat: document.querySelector('#providerCreateFormat'),
  providerCreateFormatHint: document.querySelector('#providerCreateFormatHint'),
  ...
  whitelistMonitorRecoveryConfirmations: document.querySelector('#whitelistMonitorRecoveryConfirmations'),
...
els.providerCreateFormat.addEventListener('change', handleProviderCreateFormatChange);
...
[els.whitelistMonitorInterval, els.whitelistMonitorConfirmations, els.whitelistMonitorRecoveryConfirmations, els.whitelistMonitorTimeout, els.whitelistMonitorProxyGroup, els.whitelistMonitorActionMode, els.whitelistMonitorYandexRelayEnabled]
  .forEach((control) => {
...
function handleFileSelect(event) {
  ...
  reader.onload = () => {
    ...
    state.whitelistMonitor.proposalSnapshot = null;
...
async function loadRouterConfig(options = {}) {
  ...
  try {
    ...
    state.xrayProviderAdapterUrl = String(data.xrayProviderAdapterUrl || '');
...
async function saveRouterConfig() {
  ...
  if (!await validateWhitelistProposalSnapshot()) return;
...
function startProviderStatusPolling() {
  if (typeof window.setInterval !== 'function') return;
  window.setInterval(() => {
    if (!document.hidden && state.routerApiAvailable && !state.providerStatusLoading) {
      loadProviderStatuses({ silent: true });
    }
...
function buildResourceMonitorTimeline(events, runtime, service, nowMs = Date.now(), hours = RESOURCE_MONITOR_HISTORY_HOURS) {
  ...
  for (let index = 0; index < totalHours; index += 1) {
    ...
    const inheritedState = currentState;
    const states = [];
    ...
    const stateValue = (states.length > 0 ? states : [inheritedState]).reduce(
...
function isResourceMonitorSelectionPersistenceEnabled(text) {
  ...
  const profile = findTopSection(lines, 'profile');
  if (!profile) return false;
  const entry = parseTopLevelKeyValueLine(lines[profile.start]);
  const inlineMap = parseInlineMap(entry?.value);
  if (inlineMap?.has('store-selected')) {
    return cleanScalar(inlineMap.get('store-selected')).toLowerCase() === 'true';
  }
  return lines
    .slice(profile.start + 1, profile.end)
    .some((line) => {
      const item = parseKeyValueLine(line, 2);
      return item?.key === 'store-selected' && cleanScalar(item.value).toLowerCase() === 'true';
    });
}

function createResourceMonitorProfileReplacement(lines, groupsSection) {
  if (!state.groups.some((group) => group.requiresSelectionPersistence)) return null;
  const profile = findTopSection(lines, 'profile');
  if (!profile) {
    return {
      start: groupsSection.start,
      end: groupsSection.start,
      lines: ['profile:', '  store-selected: true', ''],
    };
  }

  const header = parseTopLevelKeyValueLine(lines[profile.start]);
  const inlineMap = parseInlineMap(header?.value);
  if (inlineMap) {
    inlineMap.set('store-selected', 'true');
    const comment = splitYamlHighlightComment(lines[profile.start]).comment;
    return {
      start: profile.start,
      end: profile.start + 1,
      lines: [
        `profile: {${[...inlineMap.entries()].map(([key, value]) => `${formatKey(key)}: ${value}`).join(', ')}}${comment ? ` ${comment.trim()}` : ''}`,
      ],
    };
  }

  const block = lines.slice(profile.start, profile.end);
  const selectedIndex = block.findIndex((line, index) => (
    index > 0 && parseKeyValueLine(line, 2)?.key === 'store-selected'
  ));
  if (selectedIndex === -1) {
    block.splice(1, 0, '  store-selected: true');
  } else {
    const comment = splitYamlHighlightComment(block[selectedIndex]).comment;
    block[selectedIndex] = `  store-selected: true${comment ? ` ${comment.trim()}` : ''}`;
  }
  return { start: profile.start, end: profile.end, lines: block };
}
...
function isManagedResourceMonitorRule(rule, service) {
  const marker = String(rule?.comment || '').match(
    /#\s*webmihomo-monitor:\s*([a-z0-9_-]+)/i,
  );
  return marker?.[1]?.toLowerCase() === String(service || '').toLowerCase();
}

function hasManagedResourceMonitorRule(parts, service) {
  return getActiveRules().some((rule) => (
    isManagedResourceMonitorRule(rule, service)
    && resourceMonitorRuleMatches(rule, parts)
  ));
}

function resourceMonitorRulesNeedChanges(enabledKeys) {
  return Object.entries(RESOURCE_MONITOR_RULES).some(([service, rules]) => {
    const managedRules = getActiveRules().filter((rule) => isManagedResourceMonitorRule(rule, service));
    if (!enabledKeys.has(service)) return managedRules.length > 0;
    if (rules.some((parts) => !hasResourceMonitorRule(parts))) return true;
    return managedRules.some((rule) => !rules.some((parts) => resourceMonitorRuleMatches(rule, parts)));
  });
}

function getResourceMonitorRuleProviderMarkerService(line) {
  const marker = String(line || '').match(
    /#\s*webmihomo-monitor:\s*provider\s+([a-z0-9_-]+)\s*$/i,
  );
  return marker?.[1]?.toLowerCase() || '';
}

function getResourceMonitorRuleProviderKey(line) {
  return parseKeyValueLine(line, 2)?.key || '';
}

function getResourceMonitorRuleProviders(enabledKeys) {
  return [...enabledKeys].flatMap((service) => (
    (RESOURCE_MONITOR_RULE_PROVIDERS[service] || []).map((provider) => ({
      ...provider,
      service,
    }))
  ));
...
function formatResourceMonitorRuleProvider(provider) {
  return `  ${provider.name}: { type: http, format: mrs, behavior: domain, interval: 86400, url: ${JSON.stringify(provider.url)} } # webmihomo-monitor: provider ${provider.service}`;
}

function resourceMonitorRuleProvidersNeedChanges(enabledKeys, text) {
  const desired = getResourceMonitorRuleProviders(enabledKeys);
  const lines = splitLines(text);
  const section = findTopSection(lines, 'rule-providers');
  if (!section) return desired.length > 0;

  const providerLines = lines.slice(section.start + 1, section.end);
  const providerKeys = new Set(providerLines.map(getResourceMonitorRuleProviderKey).filter(Boolean));
  if (desired.some((provider) => !providerKeys.has(provider.name))) return true;

  const desiredByName = new Map(desired.map((provider) => [provider.name, provider]));
  return providerLines.some((line) => {
    const service = getResourceMonitorRuleProviderMarkerService(line);
    if (!service) return false;
    const provider = desiredByName.get(getResourceMonitorRuleProviderKey(line));
    return !provider || provider.service !== service || !line.includes(provider.url);
  });
}

function syncResourceMonitorRuleProviders(lines) {
  const enabledKeys = new Set(
    getActiveRules()
      .map((rule) => Object.keys(RESOURCE_MONITOR_RULES).find((service) => (
        isManagedResourceMonitorRule(rule, service)
      )))
      .filter(Boolean),
  );
  ...
  const section = findTopSection(nextLines, 'rule-providers');

  if (!section) {
    if (desired.length === 0) return nextLines;
    const rulesSection = findTopSection(nextLines, 'rules');
    const insertAt = rulesSection?.start ?? nextLines.length;
    const prefix = insertAt > 0 && nextLines[insertAt - 1].trim() ? [''] : [];
    nextLines.splice(
      insertAt,
      0,
      ...prefix,
      'rule-providers: # webmihomo-monitor: providers',
      ...desired.map(formatResourceMonitorRuleProvider),
      '',
    );
    return nextLines;
  }

  const header = parseTopLevelKeyValueLine(nextLines[section.start]);
  const headerValue = String(header?.value || '').trim();
  if (headerValue && headerValue !== '{}' && headerValue !== '[]') {
    throw new Error('Раздел rule-providers задан в одну строку и не может быть безопасно дополнен.');
  }
  if (headerValue) {
    const comment = splitYamlHighlightComment(nextLines[section.start]).comment;
    nextLines[section.start] = `rule-providers:${comment ? ` ${comment.trim()}` : ''}`;
  }

  const body = [];
  const presentKeys = new Set();
  nextLines.slice(section.start + 1, section.end).forEach((line) => {
    const service = getResourceMonitorRuleProviderMarkerService(line);
    const key = getResourceMonitorRuleProviderKey(line);
    if (!service) {
      if (key) presentKeys.add(key);
      body.push(line);
      return;
    }
    const provider = desiredByName.get(key);
    if (!provider || provider.service !== service) return;
    presentKeys.add(key);
    body.push(formatResourceMonitorRuleProvider(provider));
  });

  const missing = desired
    .filter((provider) => !presentKeys.has(provider.name))
    .map(formatResourceMonitorRuleProvider);
  let insertAt = body.length;
  while (insertAt > 0 && !body[insertAt - 1].trim()) insertAt -= 1;
  body.splice(insertAt, 0, ...missing);

  const managedHeader = /#\s*webmihomo-monitor:\s*providers\s*$/i.test(nextLines[section.start]);
  const hasProviderEntries = body.some((line) => Boolean(getResourceMonitorRuleProviderKey(line)));
  if (managedHeader && !hasProviderEntries) {
    nextLines.splice(section.start, section.end - section.start);
    return nextLines;
  }

  nextLines.splice(section.start + 1, section.end - section.start - 1, ...body);
  return nextLines;
}
...
function prepareResourceMonitorConfig(sources, services = null) {
  ...
  enabledEntries.forEach(([key]) => {
    const currentRules = RESOURCE_MONITOR_RULES[key] || [];
    getActiveRules()
      .filter((rule) => (
        isManagedResourceMonitorRule(rule, key)
        && !currentRules.some((parts) => resourceMonitorRuleMatches(rule, parts))
      ))
      .forEach((rule) => {
        rule.deleted = true;
      });
...
function renderWhitelistMonitorProposal(config, runtime) {
  ...
  const automatic = config.actionMode === 'automatic';
  const fallbackActive = automatic
    ? Boolean(runtime.fallbackActive)
  ...
  if (automatic) {
    const pending = runtime.automaticActionPending || '';
    const failed = runtime.lastConfigActionOk === false && Boolean(pending);
    const cannotActivate = runtime.state === 'confirmed' && !proxyReady && !fallbackActive;
    els.whitelistMonitorProposal.hidden = !fallbackActive && !pending && !failed && !cannotActivate;
    ...
    if (failed) {
      els.whitelistMonitorProposalBadge.textContent = 'Ошибка автозамены';
      els.whitelistMonitorProposalTitle.textContent = 'Конфиг не переключён';
      els.whitelistMonitorProposalMessage.textContent = runtime.lastConfigActionMessage || 'MihUI повторит попытку после защитной паузы.';
    } else if (pending === 'restore') {
      els.whitelistMonitorProposalBadge.textContent = 'Ожидает восстановления';
      els.whitelistMonitorProposalTitle.textContent = 'Нормальный доступ подтверждён';
      els.whitelistMonitorProposalMessage.textContent = 'Исходная маршрутизация будет восстановлена после 15-минутной защитной паузы между заменами.';
    } else if (pending === 'activate') {
      els.whitelistMonitorProposalBadge.textContent = 'Ожидает замены';
      els.whitelistMonitorProposalTitle.textContent = 'Режим белых списков подтверждён';
      els.whitelistMonitorProposalMessage.textContent = 'MihUI применит временную маршрутизацию после 15-минутной защитной паузы между заменами.';
    } else if (cannotActivate) {
      els.whitelistMonitorProposalBadge.textContent = 'PROXY не подтверждён';
      els.whitelistMonitorProposalTitle.textContent = 'Автозамена приостановлена';
      els.whitelistMonitorProposalMessage.textContent = 'Выбранная PROXY-группа не восстановила достаточно контрольных ресурсов.';
    } else {
      els.whitelistMonitorProposalBadge.textContent = 'Применено автоматически';
      els.whitelistMonitorProposalTitle.textContent = 'Временная маршрутизация активна';
      els.whitelistMonitorProposalMessage.textContent = `Остальной трафик направляется в ${config.proxyGroup}. Исходные правила вернутся после устойчивого восстановления DIRECT.`;
    }
    return;
  }
...
async function handleWhitelistMonitorProposal() {
  ...
  try {
    ...
    state.whitelistMonitor.proposalSnapshot = {
      action: fallbackActive ? 'restore' : 'activate',
    };
    state.whitelistMonitor.error = '';
    render();
    setActiveSection('review');
...
function renderWhitelistMonitorForm() {
  ...
  els.whitelistMonitorRecoveryConfirmations.value = String(settings.recoveryThreshold || 3);
...
function renderWhitelistMonitorEndpointList(kind) {
  ...
  settings[key].forEach((endpoint) => {
    ...
    if (kind === 'control') {
      ...
      const proxyCheck = document.createElement('button');
      proxy.className = 'whitelist-monitor-result';
      proxy.dataset.whitelistResult = 'proxy';
      proxyCheck.className = 'button ghost compact whitelist-monitor-proxy-check';
      proxyCheck.type = 'button';
      proxyCheck.textContent = 'Проверить PROXY';
...
function updateWhitelistMonitorEndpointResults() {
  ...
  document.querySelectorAll('[data-whitelist-endpoint-id]').forEach((row) => {
    ...
    const manual = state.whitelistMonitor.manualProxyResults[row.dataset.whitelistEndpointId];
    const manualIsCurrent = manual
      && manual.runtimeCheckedAt === Number(state.whitelistMonitor.runtime?.checkedAt || 0);
    const proxyResult = manualIsCurrent ? manual.result : result.proxy;
    ...
    updateWhitelistMonitorResult(
      row.querySelector('[data-whitelist-result="proxy"]'),
      proxyResult,
      'PROXY',
    ...
    if (proxyCheck) {
      ...
      proxyCheck.disabled = checking || row.classList.contains('is-disabled');
      proxyCheck.textContent = checking ? 'Проверяем…' : 'Проверить PROXY';
    }
  });
}

function updateWhitelistMonitorResult(element, result, label, options = {}) {
  if (!element) return;
...
function renderWhitelistMonitorEvents() {
  ...
  events.slice(0, 20).forEach((event) => {
    ...
    const tone = ['confirmed', 'config_failed'].includes(event.type)
      ? 'error'
...
async function validateWhitelistProposalSnapshot() {
  const snapshot = state.whitelistMonitor.proposalSnapshot;
  if (!snapshot) return true;
  try {
    const data = await apiJson('/api/whitelist-monitor');
    const config = data.config || defaultWhitelistMonitorClientSettings();
    const runtime = data.runtime || {};
    const proxyReady = Number(runtime.controlProxyRecoveries || 0) >= Number(config.controlFailureThreshold || 0);
    const stillValid = snapshot.action === 'activate'
      ? config.actionMode === 'suggest' && runtime.state === 'confirmed' && proxyReady && !runtime.fallbackActive
      : runtime.state === 'normal' && Boolean(runtime.fallbackActive);
    if (stillValid) return true;
    showMessage('Состояние сети изменилось после подготовки черновика. Конфиг не сохранён — подготовьте переключение заново.', {
      severity: 'warning',
    });
    return false;
  } catch (error) {
    showMessage(`Не удалось повторно проверить режим белых списков: ${error?.message || error}`, {
      severity: 'error',
    });
    return false;
  }
...
function handleWhitelistMonitorPolicyInput(event) {
  ...
  draft.recoveryThreshold = Number(els.whitelistMonitorRecoveryConfirmations.value);
...
function handleWhitelistMonitorEndpointClick(event) {
  const proxyCheckId = event.target.dataset.whitelistProxyCheck;
  if (proxyCheckId) {
    checkWhitelistMonitorProxyEndpoint(proxyCheckId);
    return;
  }
...
async function checkWhitelistMonitorProxyEndpoint(endpointId) {
  if (state.whitelistMonitor.proxyCheckingId) return;
  if (state.whitelistMonitor.dirty) {
    showMessage('Сначала сохраните изменения настроек наблюдения.', { severity: 'warning' });
    return;
  }
  state.whitelistMonitor.proxyCheckingId = endpointId;
  delete state.whitelistMonitor.manualProxyResults[endpointId];
  updateWhitelistMonitorEndpointResults();
  try {
    const data = await apiJson('/api/whitelist-monitor/proxy-check', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    state.whitelistMonitor.manualProxyResults[endpointId] = {
      result: data.result,
      runtimeCheckedAt: Number(state.whitelistMonitor.runtime?.checkedAt || 0),
    };
    if (data.result?.ok === true) {
      showMessage(`Проверка через ${data.proxyGroup}: ${Number(data.result.delay)} мс.`, { severity: 'success' });
    } else {
      showMessage(`Проверка через ${data.proxyGroup} не пройдена: ${data.result?.message || 'нет ответа'}`, { severity: 'error' });
    }
  } catch (error) {
    showMessage(`Проверка через PROXY не запущена: ${error?.message || String(error)}`, { severity: 'error' });
  } finally {
    state.whitelistMonitor.proxyCheckingId = '';
    updateWhitelistMonitorEndpointResults();
  }
}
...
function validateWhitelistMonitorDraft(settings) {
  ...
  if (!['observe', 'suggest', 'automatic'].includes(settings.actionMode || 'observe')) {
...
async function updateProviderNow(provider) {
  ...
  try {
    const result = await apiJson('/api/providers/update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: provider.name }),
    });
    if (provider.sourceFormat === 'xray-json' && result.adapter?.state === 'partial') {
      showMessage(`Подписка ${provider.name} обновлена частично: ${formatProviderAdapterStatus(result.adapter)}.`, { severity: 'warning' });
    } else {
      showMessage(`Подписка ${provider.name} отправлена на обновление.`);
    }
  } catch (error) {
    const cached = Number(getProviderStatus(provider.name)?.proxyCount) > 0;
    if (provider.sourceFormat === 'xray-json' && error?.data?.adapter?.state === 'error' && cached) {
      showMessage(`Не удалось обновить подписку ${provider.name}. Используется предыдущая версия.`, { severity: 'warning' });
    } else {
      showMessage(`Не удалось обновить подписку ${provider.name}: ${error?.message || error}`, { severity: 'error' });
    }
  } finally {
    await loadProviderStatuses({ silent: true });
    await loadNodeInventory({ silent: true });
...
function parseXrayProviderAdapterUrl(value) {
  const rawUrl = String(value || '').trim();
  if (!rawUrl) return null;
  try {
    const parsed = new URL(rawUrl);
    const host = parsed.hostname.toLowerCase();
    const isLoopback = host === 'localhost' || host === '::1' || host === '[::1]' || host === '127.0.0.1' || host.startsWith('127.');
    const sourceUrl = parsed.searchParams.get('url') || '';
    const providerName = parsed.searchParams.get('provider') || '';
    if (parsed.protocol !== 'http:' || !isLoopback || parsed.pathname !== '/mihomo/xray/provider.yaml') return null;
    if (!providerName || !/^https?:\/\/\S+$/i.test(sourceUrl)) return null;
    return {
      rawUrl,
      sourceUrl,
      providerName,
...
function normalizeXrayProviderAdapterEndpoint(value) {
  ...
  try {
    ...
    const host = parsed.hostname.toLowerCase();
    const isLoopback = host === 'localhost' || host === '::1' || host === '[::1]' || host === '127.0.0.1' || host.startsWith('127.');
    if (parsed.protocol !== 'http:' || !isLoopback || parsed.pathname !== '/mihomo/xray/provider.yaml') return '';
...
function canUseXrayProviderAdapter(provider = null) {
  return Boolean(
    normalizeXrayProviderAdapterEndpoint(state.xrayProviderAdapterUrl)
    || normalizeXrayProviderAdapterEndpoint(provider?.xrayAdapterUrl),
  );
}

function buildXrayProviderAdapterUrl(provider) {
  const endpoint = normalizeXrayProviderAdapterEndpoint(state.xrayProviderAdapterUrl)
    || normalizeXrayProviderAdapterEndpoint(provider?.xrayAdapterUrl);
  if (!endpoint) return '';
...
function getProviderOutputUrl(provider) {
  if (provider?.sourceFormat !== 'xray-json') return String(provider?.url || '');
  const currentEndpoint = normalizeXrayProviderAdapterEndpoint(state.xrayProviderAdapterUrl);
  const originalEndpoint = normalizeXrayProviderAdapterEndpoint(provider.xrayAdapterUrl);
  const unchanged = !provider.isNew
    && provider.rawUrl
  ...
  return buildXrayProviderAdapterUrl(provider) || provider.rawUrl || String(provider.url || '');
}

function snapshotProvider(provider, options = {}) {
  const output = Boolean(options.output);
  const type = String(provider.type || 'http').toLowerCase();
  const outputUrl = getProviderOutputUrl(provider);
  const writesUrl = output
    ? provider.isNew
      ? (type === 'http' || provider.hasUrl) && hasOutputValue(outputUrl)
      : provider.hasUrl
    : provider.hasUrl;
  ...
  return {
    ...
    url: output ? outputUrl : provider.rawUrl ?? provider.url ?? '',
...
function createProviderInspector(provider) {
  ...
  const sourceValues = [state.hideProviderUrls ? maskSensitiveUrl(provider.url) || 'Не указан' : provider.url || 'Не указан'];
  if (provider.sourceFormat === 'xray-json') sourceValues.push('Формат: Xray JSON · локальная обработка');
  const updateValues = [
  ...
  const adapterSummary = provider.sourceFormat === 'xray-json'
    ? formatProviderAdapterStatus(status?.adapter, status?.proxyCount)
    : '';
  if (adapterSummary) updateValues.push(adapterSummary);
  ...
  content.append(
    ...
    createProviderInspectorSection('Источник подписки', sourceValues, 'value'),
    createProviderFilterInspectorSection(provider),
    createProviderInspectorSection('Обновление', updateValues, 'list'),
  );
...
function createProviderEditor(provider, index, options = {}) {
  ...
  bindProviderSourceFormat(row, provider);
...
function formatProviderListMeta(provider) {
  ...
  if (status?.proxyCount !== null && status?.proxyCount !== undefined) {
    const format = provider.sourceFormat === 'xray-json' ? ' · Xray JSON' : '';
    return `${formatProxyCount(status.proxyCount)}${format} · ${formatProviderUpdatedAt(status.updatedAt) || 'время неизвестно'}`;
  }
...
function formatProviderListBadge(provider) {
  ...
  if (provider.sourceFormat === 'xray-json') return 'Xray JSON';
  return provider.isNew ? 'Новая' : provider.type || 'http';
}

function formatProviderAdapterStatus(adapter, proxyCount = null) {
  if (!adapter || adapter.mode !== 'xray-json') return '';
  if (adapter.state === 'partial') {
    return `${adapter.convertedCount || 0} из ${adapter.sourceCount || 0} · пропущено ${adapter.skippedCount || 0}`;
  }
  if (adapter.state === 'error') {
    return Number(proxyCount) > 0 ? 'Используется предыдущая версия' : 'Преобразование не выполнено';
  }
  if (adapter.state === 'ok') return 'Xray JSON преобразован локально';
  return '';
}
...
function renderProviderRuntimeStatus(root, provider) {
  ...
  if (provider.sourceFormat === 'xray-json') parts.push('Xray JSON');
  ...
  const adapter = provider.sourceFormat === 'xray-json' ? status.adapter : null;
  const adapterSummary = formatProviderAdapterStatus(adapter, status.proxyCount);
  if (adapter?.state === 'partial') box.classList.add('is-warning');
  if (adapter?.state === 'error') {
    box.classList.add(Number(status.proxyCount) > 0 ? 'is-warning' : 'is-danger');
  }
  if (adapterSummary) parts.push(adapterSummary);
...
function formatProviderUpdatedAt(value) {
  ...
  const date = parseProviderUpdatedAt(value);
...
function getStaleProviderInfo(provider, now = Date.now()) {
  if (!provider?.hasInterval) return null;
  const intervalSeconds = Number(provider.interval);
  if (!Number.isFinite(intervalSeconds) || intervalSeconds <= 0) return null;

  const status = getProviderStatus(provider.name);
  const updatedAt = parseProviderUpdatedAt(status?.updatedAt);
  if (!updatedAt) return null;

  const intervalMs = intervalSeconds * 1000;
  const elapsedMs = Number(now) - updatedAt.getTime();
  if (!Number.isFinite(elapsedMs) || elapsedMs <= intervalMs + PROVIDER_STALE_GRACE_MS) return null;

  return {
    updatedAt: status.updatedAt,
    intervalCount: Math.floor(elapsedMs / intervalMs),
  };
}

function getProviderRuntimeWarnings(provider, now = Date.now()) {
  const status = getProviderStatus(provider?.name);
  if (!status) return [];

  const warnings = [];
  const subscriptionInfo = status.subscriptionInfo || {};
  const expireSeconds = Number(subscriptionInfo.Expire ?? subscriptionInfo.expire);
  const nowMs = Number(now);

  if (Number.isFinite(expireSeconds) && expireSeconds > 0 && Number.isFinite(nowMs)) {
    ...
    if (remainingMs <= SUBSCRIPTION_EXPIRY_WARNING_MS) {
      warnings.push({
        ...
        text: remainingMs <= 0
          ? `Срок действия истёк ${formatSubscriptionDate(expireMs)}.`
          : `Истекает ${formatSubscriptionDate(expireMs)} · осталось ${formatRouteCount(Math.max(1, Math.ceil(remainingMs / (24 * 60 * 60 * 1000))), 'день', 'дня', 'дней')}.`,
      });
    }
  }

  const upload = Math.max(0, Number(subscriptionInfo.Upload ?? subscriptionInfo.upload ?? 0));
  const download = Math.max(0, Number(subscriptionInfo.Download ?? subscriptionInfo.download ?? 0));
  const total = Number(subscriptionInfo.Total ?? subscriptionInfo.total);
  ...
  if (Number.isFinite(total) && total > 0 && Number.isFinite(used) && used / total >= SUBSCRIPTION_TRAFFIC_WARNING_RATIO) {
    ...
    warnings.push({
      ...
      text: exhausted
        ? `Использовано ${Math.round((used / total) * 100)}% · ${formatDataSize(used)} из ${formatDataSize(total)}.`
        : `Использовано ${Math.round((used / total) * 100)}% · осталось ${formatDataSize(total - used)} из ${formatDataSize(total)}.`,
    });
  }
...
function getProviderDisplayState(provider, now = Date.now()) {
  ...
  if (provider?.sourceFormat === 'xray-json' && status?.adapter?.state === 'error' && (!hasNodeCount || Number(status.proxyCount) <= 0)) {
    return {
      attention: null,
      className: 'is-danger',
      detail: status.adapter.message || 'Преобразование Xray JSON не выполнено',
      label: 'Не работает',
    };
  }
...
function formatDataSize(value) {
  ...
  return `${size.toLocaleString('ru-RU', { maximumFractionDigits: size < 10 && unitIndex > 0 ? 1 : 0 })} ${units[unitIndex]}`;
}
...
function renderProviderCreateDialog() {
  ...
  els.providerCreateFormat.value = draft.sourceFormat;
  const xrayOption = els.providerCreateFormat.querySelector('option[value="xray-json"]');
  if (xrayOption) xrayOption.disabled = !canUseXrayProviderAdapter();
  renderProviderCreateFormatHint();
...
function handleProviderCreateFormatChange() {
  const draft = state.providerCreateDraft;
  if (!draft) return;
  draft.sourceFormat = els.providerCreateFormat.value === 'xray-json' ? 'xray-json' : 'direct';
  renderProviderCreateFormatHint();
  syncProviderCreateValidation();
}
...
function getProviderCreateUrlError(value, sourceFormat = 'direct') {
...
function syncProviderCreateValidation({ showErrors = false } = {}) {
  ...
  const urlError = getProviderCreateUrlError(draft.url, draft.sourceFormat);
  const nameError = getProviderCreateNameError(draft.name);
  const formatUnavailable = draft.sourceFormat === 'xray-json' && !canUseXrayProviderAdapter();
  ...
  els.providerCreateSubmitButton.disabled = Boolean(urlError || nameError || formatUnavailable || !intervalValid || !healthIntervalValid);
  return !els.providerCreateSubmitButton.disabled;
}

function commitProviderCreateDraft(draft) {
  if (!draft || getProviderCreateUrlError(draft.url, draft.sourceFormat) || getProviderCreateNameError(draft.name)) return null;
  const intervalDefaults = getProviderIntervalDefaults();
  return addProvider({
...
function bindProviderSourceFormat(root, provider) {
  const select = root.querySelector('.provider-source-format');
  const hint = root.querySelector('.provider-source-format-hint');
  const xrayOption = select.querySelector('option[value="xray-json"]');
  const available = canUseXrayProviderAdapter(provider);
  select.value = provider.sourceFormat === 'xray-json' ? 'xray-json' : 'direct';
  ...
  const renderHint = () => {
    if (select.value === 'xray-json') {
      ...
      hint.textContent = 'Локальное преобразование Xray JSON доступно в MihUI на роутере.';
    }
  };
  renderHint();
  select.addEventListener('change', () => {
    updateProvider(provider, 'sourceFormat', select.value === 'xray-json' ? 'xray-json' : 'direct');
    renderHint();
  });
}
...
function generateOutput() {
  ...
  const resourceMonitorProfileReplacement = createResourceMonitorProfileReplacement(lines, groupsSection);
  if (resourceMonitorProfileReplacement) replacements.push(resourceMonitorProfileReplacement);
  const hasActiveProviders = state.providers.some((provider) => !provider.deleted);
...
function serializeProvider(provider) {
  ...
  if (changed('hasUrl', 'url')) {
    setOptionalNestedScalar(lines, 1, 'url', current.hasUrl ? current.url : '');
  }
...
function createProviderBlock(provider) {
  ...
  if (type === 'http' || provider.hasUrl || provider.url) {
    lines.push(`    url: ${formatScalar(outputUrl)}`);
  }
...
function serializeRule(rule, renamedGroups = new Map()) {
  ...
  const body = buildRuleParts(nextRule).join(',');
...
function parseProviders(lines, section) {
  ...
  while (index < section.end) {
    ...
    const rawUrl = readScalar(rawLines, 4, 'url') || '';
