...
els.mihomoUiUpdateButton.addEventListener('click', updateMihomoUi);
...
async function updateMihomoUi() {
  setMihomoUiUpdateButton(true, 'Обновление...');

  try {
    if (window.location?.protocol === 'file:') {
      throw new Error('откройте страницу через Mihomo external-ui');
    }

    let response = await requestMihomoUiUpgrade(getStoredMihomoSecret());
    if (response.status === 401 || response.status === 403) {
      const secret = window.prompt('Secret Mihomo API') || '';
      if (!secret.trim()) throw new Error('нужен secret Mihomo API');
      setStoredMihomoSecret(secret.trim());
      response = await requestMihomoUiUpgrade(secret.trim());
    }

    if (!response.ok) {
      throw new Error(await readMihomoUiUpdateError(response));
    }

    showMessage('Обновление UI запущено через Mihomo. Если версия уже скачалась, обновите страницу через несколько секунд.');
    window.setTimeout(() => {
      try {
        window.location.reload();
      } catch (error) {
        // Page reload is only a convenience after Mihomo finishes updating assets.
      }
    }, 4000);
  } catch (error) {
...
async function requestMihomoUiUpgrade(secret) {
  const headers = {};
  if (secret) headers.Authorization = `Bearer ${secret}`;
  return fetch('/upgrade/ui', {
    method: 'POST',
    headers,
    cache: 'no-store',
  });
...
async function readMihomoUiUpdateError(response) {
...
function getStoredMihomoSecret() {
  try {
    return window.sessionStorage.getItem('webmihomo.mihomoSecret') || '';
  } catch (error) {
    return '';
  }
}

function setStoredMihomoSecret(secret) {
  try {
    window.sessionStorage.setItem('webmihomo.mihomoSecret', secret);
  } catch (error) {
    // Secret storage is optional; the update request can still use this value once.
  }
}
